...
public class UnitDaoImpl extends HibernateDaoSupport implements IUnitDao {
	...
	public void save(Object obj) {
		...
		Unit unit = (Unit)obj;
		SQLQuery query = getSession().createSQLQuery("INSERT INTO unit (id_unit, address_ip, last_update_dt, max_process_no, overload_flg, type) VALUES (?,?,?,?,?,?)");
	...
	public void setOverload(Integer id) {
		SQLQuery query = getSession().createSQLQuery("UPDATE unit SET overload_flg = true WHERE id_unit = ?");
	...
	public void setFree(Integer id) {
		SQLQuery query = getSession().createSQLQuery("UPDATE unit SET overload_flg = false WHERE id_unit = ?");
		query.setInteger(0, id);
		query.executeUpdate();
	}
	...
	public void ping(Integer id) {
		SQLQuery query = getSession().createSQLQuery("UPDATE unit SET last_update_dt = ? WHERE id_unit = ?");
	...
	public void removeUnit(Integer id) {
		SQLQuery query = getSession().createSQLQuery("DELETE FROM unit WHERE id_unit = ?");
		query.setInteger(0, id);
		query.executeUpdate();
	}
